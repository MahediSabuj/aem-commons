 ...
 */
package com.aem.commons.it.tests;
 ...
 * validates all linked resources (images, client-libs etc.).
 * 
 */
public class PublishPageValidationIT {
    ...
    private static void verifyPage (HtmlUnitClient client, String path) throws IOException {
        ...
        Assert.assertEquals("Request to [" + get.getURI().toString() + "] does not return expected return code 200",
    ...
    private static void verifyLinkedResources(HtmlUnitClient client, String path) throws ClientException, IOException, URISyntaxException {
        ...
        Assert.assertFalse(path + " does not contain any references!", references.isEmpty());
        for (URI ref : references ) {
            if (isSameOrigin(client.getUrl(), ref)) {
                LOG.info("verifying linked resource {}", ref);
                SlingHttpResponse response = client.doGet(ref.getPath());
                int statusCode = response.getStatusLine().getStatusCode();
                int responseSize = response.getContent().length();
                Assert.assertEquals("Unexpected status returned from [" + ref + "]", 200, statusCode);
